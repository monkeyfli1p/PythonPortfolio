...
def play(word):
    ...
    while True:
        ...
        if word_completion != word and tries != 0:
            ...
            if n.lower() not in word_completion and n.isalpha() and len(n) == 1:
                if n.lower() in word:
                    print("Вы угадали букву!")
                    repl = [i for i in range(len(word)) if word.startswith(n.lower(), i)]
                    for i in repl:
                        word_completion[i] = n.lower()
                    print(*word_completion, sep='')
                else:
                    print("Не угадали...")
                    tries -= 1
                    print(display_hangman(tries))
            elif n.lower() in word_completion:
...
while True:
    play(get_word())
    n = input("Хотите сыграть еще раз?(y/n): ")
    if n == 'y':
        continue
    else:
        break
